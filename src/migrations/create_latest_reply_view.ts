...
export const up = async () => {
    const query = `
        CREATE MATERIALIZED VIEW IF NOT EXISTS latest_reply AS
            ...
            PRIMARY KEY (video_id, parent_id, created_at, comment_id)
...
export const down = async () => {
    const query = `DROP MATERIALIZED VIEW IF EXISTS latest_reply;`;
    await executeQuery(query);
};